...
df = pd.read_csv('../data/given/france/france_new_avg_cases_2022.csv', parse_dates=[0], dayfirst=False) 
...
counts, bin_edges, _ = plt.hist(df['date'], weights=df['daily_avg_cases'], bins=num_bins, edgecolor='black')

plt.xlabel('Dates')
plt.ylabel('reported cases')
plt.title(f'Histogram of reported cases over Time (2022-01-01 to 2022-07-01) with {num_bins} bins')
...
df = pd.read_csv('../data/sonar_out_fr.csv', parse_dates=['date'], dayfirst=False) 
